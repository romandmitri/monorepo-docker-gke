...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/src/common/components/shadcn/dropdown-menu.tsx";
import { HoverCard, HoverCardContent, HoverCardTrigger } from "@/src/common/components/shadcn/hover-card.tsx";
import { InputGroup, InputGroupAddon, InputGroupButton } from "@/src/common/components/shadcn/input-group.tsx";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/src/common/components/shadcn/select.tsx";
import type { ChatStatus, FileUIPart } from "ai";
import TextareaAutosize, { type TextareaAutosizeProps } from "react-textarea-autosize";
import { ImageIcon, Loader2Icon, MicIcon, PaperclipIcon, PlusIcon, SendIcon, XIcon } from "lucide-react";
...
export type PromptInputTextareaProps = TextareaAutosizeProps & {
	placeholder?: string;
	ref?: RefObject<HTMLTextAreaElement | null>;
};

export const PromptInputTextarea = ({ ref, onChange, className, placeholder = "What would you like to know?", ...props }: PromptInputTextareaProps) => {
	const controller = useOptionalPromptInputController();
	const attachments = usePromptInputAttachments();
	...
	const handleKeyDown: KeyboardEventHandler<HTMLTextAreaElement> = (e) => {
		...
		if (e.key === "Backspace" && e.currentTarget.value === "" && attachments.files.length > 0) {
			e.preventDefault();
			// TODO: reidenzon - This came with broken typescript... does this actually work?!
			// @ts-ignore
	...
	return (
		<TextareaAutosize
			ref={ref}
			className={cn(
				"w-full min-w-0 resize-none rounded-none border-0 bg-transparent px-3 py-3 shadow-none",
				"focus-visible:ring-0 focus-visible:outline-none dark:bg-transparent",
...
export const PromptInputSpeechButton = ({ className, textareaRef, onTranscriptionChange, ...props }: PromptInputSpeechButtonProps) => {
	...
	return (
		<PromptInputButton
			className={cn("relative transition-all duration-200", isListening && "bg-primary text-primary-foreground animate-pulse", className)}
