	...
	"group/item flex items-center border border-transparent text-sm rounded-md transition-colors [a]:hover:bg-accent/50 [a]:transition-colors duration-100 flex-wrap outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]",
	{
		variants: {
			variant: {
				...
				"outline-developer": "border-developer dark:border-developer/40",
				"outline-positive": "border-positive dark:border-positive/40",
				muted: "bg-muted/50",
			},
			size: {
				default: "p-4 gap-4 ",
...
function ItemContent({ className, ...props }: React.ComponentProps<"div">) {
	return <div data-slot="item-content" className={cn("flex flex-1 flex-col flex-wrap gap-2 [&+[data-slot=item-content]]:flex-none", className)} {...props} />;
}
...
function ItemActions({ className, ...props }: React.ComponentProps<"div">) {
	return <div data-slot="item-actions" className={cn("flex flex-wrap items-center gap-2", className)} {...props} />;
}

function ItemHeader({ className, ...props }: React.ComponentProps<"div">) {
	return <div data-slot="item-header" className={cn("flex basis-full flex-wrap items-center justify-between gap-2", className)} {...props} />;
}
